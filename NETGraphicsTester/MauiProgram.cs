...
namespace NETGraphicsTester
{
    public static class MauiProgram
    {
        public static MauiApp CreateMauiApp()
        {
            ...
            var runtimeCoreLocation = AppDomain.CurrentDomain.GetAssemblies()
                .FirstOrDefault(a => a.GetName().Name == "Esri.ArcGISRuntime")?.Location;

            Debug.WriteLine($"ArcGIS Runtime loaded from: {runtimeCoreLocation}");

            // Also check native DLL location
            var currentDir = AppContext.BaseDirectory;
            var runtimeCorePath = Path.Combine(currentDir, "runtimecore.dll");
            if (File.Exists(runtimeCorePath))
            {
                var fileInfo = new FileInfo(runtimeCorePath);
                Debug.WriteLine($"runtimecore.dll size: {fileInfo.Length} bytes, modified: {fileInfo.LastWriteTime}");
            }
